...
class PorEstadoForm(ModelForm):
    class Meta:
        model = PorEstado
        fields = ['nome', 'modelo', 'ticket', 'base', 'facial', 'webcam', 'ativacao', 'nfe']      
        widgets = {
...
class ImplantacaoForm(ModelForm):
    class Meta:
        model = Implantacao
        fields = ['nome', 'modelo', 'ticket', 'base', 'webcam', 'ativacao', 'nfe','status', 'informacao']
        widgets = {
            'nome': forms.Select(attrs={'class': 'form-select'}),
            'modelo': forms.Select(attrs={'class': 'form-control'}),
            'ticket': forms.NumberInput(attrs={'class': 'form-control'}),
...
class ObrasAtivasForm(ModelForm):
    class Meta:
        model = ObrasAtivas
        fields = ['nome', 'construtora', 'modelo']
        widgets = {
            'nome': forms.Select(attrs={'class': 'form-select'}),
...
class ObrasDesativadasForm(ModelForm):
    class Meta:
        ...
        fields = ['nome', 'construtora', 'desativacao']
        widgets = {
            'nome': forms.Select(attrs={'class': 'form-select'}),
            'construtora': forms.Select(attrs={'class': 'form-select'}),
